...
class EmiratesNdcBookingFlight extends StatefulWidget {
  ...
  const EmiratesNdcBookingFlight({
    super.key,
  ...
  @override
  State<EmiratesNdcBookingFlight> createState() => _EmiratesNdcBookingFlightState();
}

class _EmiratesNdcBookingFlightState extends State<EmiratesNdcBookingFlight> {
...
// Replace the ElevatedButton in _buildBottomBar() with this updated version:

// Update your Book Now button in emirates_ndc_booking_flight.dart
// Replace the entire onPressed callback with this:
  ...
  onPressed: () async {
    if (_formKey.currentState!.validate() && termsAccepted) {
      try {
        ...
        // Step 1: Create Emirates NDC PNR
        Map<String, dynamic>? pnrResponse;

        try {
          debugPrint('\n🎫 === INITIATING EMIRATES NDC PNR CREATION ===');
          debugPrint('Flight Details:');
          debugPrint('  Offer ID: ${widget.selectedPackage.offerId}');
          debugPrint('  Package: ${widget.selectedPackage.name}');
          debugPrint('  Total Price: ${widget.currency} ${widget.totalPrice}');
          ...
          debugPrint('Success: ${pnrResponse['success']}');
          
          Get.back(); // Close loading dialog

          if (pnrResponse['success'] == true) {
            debugPrint('✅ PNR Created Successfully!');
            debugPrint('  PNR: ${pnrResponse['pnr']}');
            debugPrint('  Order ID: ${pnrResponse['orderId']}');
            ...
            final errorMessage = pnrResponse['error'] ?? 'Failed to create PNR';
            debugPrint('❌ PNR Creation Failed: $errorMessage');
            
            Get.snackbar(
              'Error',
              'PNR creation failed: $errorMessage',
          ...
          debugPrint('❌ Exception during PNR creation: $e');
        ...
        debugPrint('❌ Unexpected error in booking process: $e');
        debugPrint('Stack trace: $stackTrace');
...
// Don't forget to add this import at the top of emirates_ndc_booking_flight.dart:
// import 'package:ready_flights/views/flight/booking_flight/emirates_ndc/emirates_booking_details_screen.dart';
],
        ),
      ),
    );
