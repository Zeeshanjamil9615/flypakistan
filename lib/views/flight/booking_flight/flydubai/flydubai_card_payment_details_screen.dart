...
class FlyDubaiCardPaymentDetailsScreen extends StatelessWidget {
  ...
  const FlyDubaiCardPaymentDetailsScreen({
    super.key,
  ...
  Widget build(BuildContext context) {
    ...
    final departureRaw = firstLeg?['departure']?['time']?.toString();
    final departure = departureRaw != null ? DateTime.parse(departureRaw) : DateTime.now();
    final formattedDate = DateFormat('EEE, dd MMM yyyy').format(departure);
  ...
  Future<void> _handleCardPayment(double netTotal) async {
    ...
    final transactionId = 'FK_FLYDUBAI_${DateTime.now().millisecondsSinceEpoch}';
    const callbackUrl = 'readyflights://payment-success';
    final description = 'Flight Booking - FlyDubai - $transactionId';

    Get.put(
      FlyDubaiFlightPaymentData(
        flight: flight,
...
class FlyDubaiFlightPaymentData {
  final FlydubaiFlight flight;
  final FlydubaiFlight? returnFlight;
  final FlydubaiFlightFare outboundFare;
  final FlydubaiFlightFare? returnFare;
